...
class EmotionCRNN(nn.Module):
    def __init__(self, num_classes=7):  # 7 emotions for EmotionAlly
        super(EmotionCRNN, self).__init__()
        
        # MFCC branch - Reduced complexity for EmotionAlly
        ...
        # Shared layers (64 + 32 = 96 channels after concatenation)
        ...
        # LSTM: 96 hidden units
        self.lstm = nn.LSTM(128, 96, bidirectional=True, batch_first=True) 
        self.lstm_dropout = nn.Dropout(0.5)
        
        # Attention components for 96-dim LSTM
        self.prosody_projection = nn.Linear(32, 96)
        ...
        # Fully connected layers for 192-dim input
    ...
    def prosody_aware_attention(self, lstm_output, prosody_features):
        ...
        context_expanded = torch.cat([context, context], dim=-1)
        
        # Gating with matching dimensions (192)
    ...
    def forward(self, inputs):
        ...
        attended, _ = self.prosody_aware_attention(lstm_out, prosody_features)
...
class EmotionRecognitionApp(ctk.CTk):
    def __init__(self):
        ...
        # Configure window
        self.title("Speech Emotion Recognition - EmotionAlly Dataset")
        self.geometry("1200x800")
        
        # Emotion labels and colors for EmotionAlly (7 emotions, no 'calm')
    ...
    def setup_ui(self):
        ...
        title_label = ctk.CTkLabel(
            header_frame,
            text="Speech Emotion Recognition in Mental Health Podcasts\nUsing Multi-Modal CRNN with Prosody-Aware Attention",
        ...
        subtitle_label = ctk.CTkLabel(
            header_frame,
            text="EmotionAlly Dataset - 7 Emotions",
        ...
        ctk.CTkLabel(
            model_frame,
            text="EmotionAlly CRNN (7 Emotions)",
        ...
        ctk.CTkLabel(
            model_frame,
            text="Reduced complexity for mental health podcasts",
        ...
        device_label = ctk.CTkLabel(
            footer_frame,
            text=f"🖥️ Running on: {self.device} | Dataset: EmotionAlly",
            font=ctk.CTkFont(size=11),
            text_color="#808080"
        )
    ...
    def extract_mfcc(self, file_path, n_mfcc=40):
        try:
            # Load 10 seconds for EmotionAlly (longer podcast clips)
    ...
    def extract_prosodic_features(self, file_path):
        try:
            # Load 10 seconds for EmotionAlly
            y, sr = librosa.load(file_path, duration=10)
            if len(y) < sr * 10:
                y = np.pad(y, (0, sr * 10 - len(y)))
            else:
    ...
    def load_model(self):
        if self.model is None:
            try:
                self.model = EmotionCRNN(num_classes=7).to(self.device)
                
                # Try to load EmotionAlly-specific model weights
                model_paths = [
                    'best_emotionally_only_model.pth',
                    'new model/best_emotionally_only_model.pth',
                    'best_emotion_model_emotionally.pth',
                    'new model/best_emotion_model_emotionally.pth'
                ]
                ...
                for model_path in model_paths:
                    try:
                        ...
                        print(f"✓ EmotionAlly model loaded successfully from: {model_path}")
                        model_loaded = True
                        break
                ...
                if not model_loaded:
                    messagebox.showwarning(
                        "Model Not Found",
                        "EmotionAlly pre-trained model not found in any of the expected locations:\n" +
                        "\n".join(model_paths) +
                        "\n\nUsing randomly initialized model.\nPlease train the EmotionAlly model first for accurate predictions."
                    )
